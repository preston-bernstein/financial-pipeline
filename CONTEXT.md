...
**JournalEntry** — One LLM-generated narrative per calendar month, written using the Karpathy wiki pattern (an LLM periodically rewrites a running summary document, credited to Andrej Karpathy). The materializer calls the Ollama broker after recomputing spending to generate/refresh the current month's entry. UNIQUE(month_key). Exposed via the `read_financial_journal` MCP tool.
...
Where `monthly_net` = take-home pay after taxes and the pre-tax 401(k) deduction. Defined in `pipeline.config.toml`. The `get_derived_ceiling` MCP tool exposes this.
...
— it handles live Plaid/brokerage data, and the `redact` deny-list built into pino —
keyed by log field name, defined in
...
- MCP server on NAS port 3101, using Streamable HTTP (`/mcp`), bearer-token auth + Host/Origin allowlisting (`MCP_AUTH_TOKEN`, `MCP_ALLOWED_HOSTS`, `MCP_ALLOWED_ORIGINS` in `.env`)
